...
```{r}
  ...
  mt3pl <- mirt(score_mt, model = m3pl, TOL = .001, itemtype='3PL')
  
 
  summary( mt3pl )
  coef( mt3pl , simplify=TRUE, IRTpars=TRUE) %>% view
  
  pars_MT_bilog <- read.table("pars_MT_bilog.csv", sep = ";", dec = ",", header = TRUE)
  
  pars_MT_mirt <- coef(mt3pl , simplify=TRUE, IRTpars=TRUE)
  
  paras_mt <- bind_cols(pars_MT_bilog, as.data.frame(pars_MT_mirt$items))

  corr.test( paras_mt[ , c(5, 7, 11, 17:19)])
